...
async def add_token(requestor: Requestor, user_id: int, unhashed_token: str, access_id: int):
    """Add an API token for a user."""
    await self.db.execute(f"SELECT public.addtoken({user_id}, '{unhashed_token}', {access_id})")
