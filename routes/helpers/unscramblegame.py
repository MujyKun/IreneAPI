...
async def get_all_uss(requestor: Requestor) -> dict:
    ...
    return await self.db.fetch("SELECT * FROM unscramblegame.getuss")
...
async def get_us(requestor: Requestor, game_id: int) -> dict:
    ...
    return await self.db.fetch_row(
        "SELECT * FROM unscramblegame.getuss WHERE gameid = $1", game_id
    )
...
@check_permission(permission_level=DEVELOPER)
async def add_us(
    requestor: Requestor, date_id, status_ids, mode_id, difficulty_id
) -> dict:
