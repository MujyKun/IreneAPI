...
class LackingPermissions(BaseError):
    ...
    def __init__(self):
        super(LackingPermissions, self).__init__(status_code=403, message="You are lacking permissions.")


class InvalidLogin(BaseError):
    """Raised when an Invalid Login occurs."""
    def __init__(self):
        super(InvalidLogin, self).__init__(status_code=401, message="Could not authenticate the login.")


class BadRequest(BaseError):
    """Raised when there is a bad request (missing information or bad input)"""
    def __init__(self):
        super(BadRequest, self).__init__(status_code=400,
